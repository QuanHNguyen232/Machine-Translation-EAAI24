...
class PivotTrans(nn.Module):
    def __init__(self, cfg, models: list, device):
        super().__init__()
        self.cfg = copy.deepcopy(cfg)
        self.cfg.pop('seq2seq', '')
        self.cfg.pop('tri', '')
        self.cfg['model_id'] = self.modelname = 'pivTrans_' + cfg['model_id']
    ...
    def add_submodels(self, models: list):
        ...
        for i in range(1, self.num_model):
            ...
            assert isinstance(models[i-1], Seq2SeqTransformer), f'{type(models[i-1])} != Seq2SeqTransformer'
            assert isinstance(models[i], Seq2SeqTransformer), f'{type(models[i])} != Seq2SeqTransformer'
        # add submodel
    ...
    def translate(self, src_sentence: str, tkzer_dict, field_dict):
        ...
        for i in range(self.num_model):
            # GET MODEL
            submodel = getattr(self, f'model_{i}') # Seq2Seq model already sort src by src_len in forward
            res = submodel.translate(src_sentence, tkzer_dict, field_dict)
            text, toks = res['results'], res['tokens']
            src_sentence = ' '.join(text).replace("<sos>", "").replace("<eos>", "").split()
        return {"results": text, "tokens": toks}